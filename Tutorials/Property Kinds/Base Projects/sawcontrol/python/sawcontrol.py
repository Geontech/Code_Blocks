...
# AUTO-GENERATED
#
# Source: sawcontrol.spd.xml
from ossie.resource import start_component
import logging

from sawcontrol_base import *

class sawcontrol_i(sawcontrol_base):
    ...
    def process(self):
        ...
        Messages:
            ...
            self.port_input.registerMessage("my_msg", sawcontrol_i.MyMsg, self.msg_callback)
        ...
        if (time.time() - self.lastUpdate > 1.0):
            ...
            self.lastUpdate = time.time()
            self.freq = self.freq + 2000.0
            if (50000.0 <= self.freq):
                self.freq = 10000.0
